...
3. If `de_lever` is **true**: `schwab.py positions`, then SELL the FULL leveraged position (TQQQ or UPRO — whole shares, the entire holding). Preview every order (`schwab.py preview-order`); warning -> skip, log, alert. Confirm each fill via `schwab.py orders filled` (FILL TRUTH).
