...
9. JOURNAL — append exactly ONE compact JSON line to ~/.claude/skills/genesis-exodus-schwab/state/journal.jsonl (ABSOLUTE path) with "ts" = ISO-8601 WITH local offset, e.g. "2026-08-27T11:15:00-0400" — run `date '+%Y-%m-%dT%H:%M:%S%z'` and paste the RESULT as a literal string. NEVER write a shell variable like $TS into the JSON: an unexpanded var makes the entry unparseable and the missed-run watchdog then reports a false stall, run_type "quick_check", liquidationValue, cashAvailableForTrading, per-position list, fills, orders, decision.
