...
# HRHR's account is pinned IN CODE as well as in schwab.env. The env value alone is a
...
def _account_hash():
    ...
    if not want or want.startswith("<"):
        _err("SCHWAB_TRADING_ACCOUNT not set — refusing to trade without an explicit "
             "account whitelist")
    if HRHR_ACCOUNT.startswith("<"):
        _err("HRHR_ACCOUNT is still the <UNASSIGNED> placeholder — the third Schwab "
             "account has not been created/pinned yet. Refusing to trade.")
    if want != HRHR_ACCOUNT:
        _err(f"account mismatch: schwab.env says {want} but this is the HRHR skill, "
             f"pinned to {HRHR_ACCOUNT}. Refusing to act on another strategy's account.")
