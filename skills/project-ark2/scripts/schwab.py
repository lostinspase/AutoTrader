...
# ARK2's account is pinned IN CODE as well as in schwab.env. The env value alone is a
# single point of failure: a typo pointing at Genesis (…3393) or Babel (…5301) would
...
def _account_hash():
    ...
    if not want or want.startswith("<"):
        _err("SCHWAB_TRADING_ACCOUNT not set — refusing to trade without an explicit "
             "account whitelist")
    if ARK2_ACCOUNT.startswith("<"):
        _err("ARK2_ACCOUNT is still the <UNASSIGNED> placeholder — the third Schwab "
             "account has not been created/pinned yet. Refusing to trade.")
    if want != ARK2_ACCOUNT:
        _err(f"account mismatch: schwab.env says {want} but this is the ARK2 skill, "
             f"pinned to {ARK2_ACCOUNT}. Refusing to act on another strategy's account.")
